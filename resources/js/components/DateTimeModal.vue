...
<template>
  <div v-if="modelValue" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
    <div class="bg-white p-6 rounded-xl shadow-lg w-[90%] md:w-[700px] relative">
      ...
      <!-- Close button -->
      <button @click="$emit('update:modelValue', false)" class="absolute top-3 right-5">✕</button>
      ...
      <div class="flex flex-col md:flex-row gap-10">
        ...
        <div class="flex-1 p-2 border-2 rounded-xl border-black">
          ...
          <div class="grid grid-cols-7 gap-2">
            <div
              ...
              :class="{
                'bg-red-300 cursor-not-allowed': isUnavailable(day),
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-col space-y-2">
            <label v-for="slot in timeSlots" :key="slot.value" class="flex items-center space-x-2">
              <input
                type="radio"
                :value="slot.value"
      ...
      <div class="mt-6 flex justify-end">
        <button @click="confirmSelection" class="bg-neutral text-white px-6 py-2 rounded-full">
          Confirm
        </button>
      </div>
    </div>
